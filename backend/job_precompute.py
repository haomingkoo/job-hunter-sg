...
def apply_job_precomputes(job_data: dict) -> dict:
    ...
    relationship = classify_employer_relationship(
        source=job_data.get("source", "") or "",
        company=job_data.get("company", "") or "",
        agency=job_data.get("agency", "") or "",
        ssic_code=job_data.get("company_ssic_code", "") or "",
        ssic_description=job_data.get("company_ssic_description", "") or "",
        ssic_source=job_data.get("company_ssic_source", "") or "",
        description=job_data.get("description", "") or "",
    )
    job_data["employer_relationship"] = relationship.relationship
    job_data["employer_relationship_evidence"] = relationship.evidence
    job_data["direct_employer"] = int(relationship.relationship != EMPLOYER_RELATIONSHIP_INTERMEDIARY)
    return job_data
...
def rollup_company_promotional_scores(db) -> dict:
    ...
    flagged = func.sum(case((ScrapedJob.promotional_score >= PROMOTIONAL_THRESHOLD, 1), else_=0))
    total = func.count(ScrapedJob.id)
    rows = (
        apply_public_job_visibility(db.query(ScrapedJob.company, total.label("total"), flagged.label("flagged")))
    ...
    apply_public_job_visibility(db.query(ScrapedJob)).filter(ScrapedJob.company_promotional_score != 0).update(
        {ScrapedJob.company_promotional_score: 0}, synchronize_session=False
    )
    for company, score in tainted.items():
        apply_public_job_visibility(db.query(ScrapedJob)).filter(ScrapedJob.company == company).update(
