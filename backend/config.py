...
CANDIDATE_PROFILE_MODEL_HTTP_TIMEOUT_SECONDS: int = _positive_int_env(
    "CANDIDATE_PROFILE_MODEL_HTTP_TIMEOUT_SECONDS",
    90,
)
CANDIDATE_PROFILE_MODEL_TRANSPORT_RETRIES: int = _nonnegative_int_env(
    "CANDIDATE_PROFILE_MODEL_TRANSPORT_RETRIES",
    0,
)
...
CANDIDATE_PROFILE_CORRECTION_ATTEMPTS: int = _positive_int_env(
    "CANDIDATE_PROFILE_CORRECTION_ATTEMPTS",
    3,
)
_CANDIDATE_PROFILE_MAX_STAGE_SECONDS = (
    CANDIDATE_PROFILE_MODEL_HTTP_TIMEOUT_SECONDS
    * (CANDIDATE_PROFILE_MODEL_TRANSPORT_RETRIES + 1)
    * max(
        CANDIDATE_PROFILE_VALIDATION_ATTEMPTS,
        CANDIDATE_PROFILE_REVIEW_ATTEMPTS,
        CANDIDATE_PROFILE_CORRECTION_ATTEMPTS,
...
if RECRUITMENT_RUN_LEASE_SECONDS <= _CANDIDATE_PROFILE_MAX_STAGE_SECONDS:
    raise ValueError(
        "RECRUITMENT_RUN_LEASE_SECONDS must exceed the full candidate-profile attempt "
        f"envelope ({_CANDIDATE_PROFILE_MAX_STAGE_SECONDS}s), got "
        f"{RECRUITMENT_RUN_LEASE_SECONDS}"
    )
...
CRAWL_MAX_NEW_JOBS_PER_SOURCE: int = _positive_int_env(
    "CRAWL_MAX_NEW_JOBS_PER_SOURCE",
    10000,
)
CRAWL_VISIBLE_HASH_BACKFILL_BATCH: int = _positive_int_env(
    "CRAWL_VISIBLE_HASH_BACKFILL_BATCH",
    5000,
)
CRAWL_LEGACY_HIDDEN_PRUNE_BATCH: int = _nonnegative_int_env(
    "CRAWL_LEGACY_HIDDEN_PRUNE_BATCH",
    0,
)
